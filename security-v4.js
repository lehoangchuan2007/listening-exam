...
(function(){
  const protectedPage=/admin\.html$|manage\.html$|results\.html$/.test(location.pathname);
  ...
  async function teacherCheck(force=false){
    try{
      ...
      checkedResult=!error&&!!data;
      return checkedResult;
    }catch(error){
      console.error('Teacher permission check failed:',error);
      clearCache();
      return false;
    }
  }
  ...
  client.auth.onAuthStateChange((event,session)=>{
    clearCache();
    if(event==='SIGNED_OUT'){
      if(!redirecting)location.replace('./admin.html');
      return;
    }
    if(event==='SIGNED_IN'&&session){setTimeout(()=>{if(!redirecting)requireTeacher();},0);}
  });

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',requireTeacher,{once:true});
  else requireTeacher();
})();
