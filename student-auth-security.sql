...
-- Run this file ONCE in Supabase SQL Editor after the student Auth UI is deployed.
-- It makes the student-facing exam RPCs require an authenticated account and
-- binds every new submission to auth.uid().

begin;

-- Store the authenticated student who submitted each new attempt.
...
-- Helper used by student-facing RPCs.
...
-- Do not expose student exam RPCs to anonymous visitors.
do $$
begin
  ...
  if to_regprocedure('public.get_reading_exam_for_student(uuid)') is not null then
    revoke execute on function public.get_reading_exam_for_student(uuid) from anon;
    grant execute on function public.get_reading_exam_for_student(uuid) to authenticated;
  end if;
  if to_regprocedure('public.get_public_exam(uuid)') is not null then
    revoke execute on function public.get_public_exam(uuid) from anon;
    grant execute on function public.get_public_exam(uuid) to authenticated;
  end if;
end $$;

-- Reading RPC: require a student session even though it is security-definer.
...
-- Public exam RPC is kept for compatibility with older pages, but now also
...
begin
  ...
  select jsonb_build_object(
    ...
    'duration_minutes',e.duration_minutes,'audio_url',e.audio_url,
    'audio_name',e.audio_name,'questions',e.questions,
    'start_at',e.start_at,'end_at',e.end_at,'max_attempts',e.max_attempts
  ) into result
  from public.exams e
...
-- Server-side submission: the account's Auth metadata is authoritative.
-- The name/MSSV parameters are retained for backwards compatibility with the
-- existing front-end, but are deliberately ignored for identity purposes.
...
begin
  ...
  if canonical_sid='' then raise exception 'Student ID is missing'; end if;
  ...
  insert into public.submissions(
    exam_id,student_user_id,student_name,student_id,answers,
    correct_count,total_questions,score
  ) values(
    p_exam_id,auth.uid(),canonical_name,canonical_sid,submitted_answers,
    correct,total,score_value
...
-- History for the currently logged-in student only. Answer keys are exposed
-- only for that student's own submitted attempts.
