...
window.SUPABASE_CONFIG = {
  ...
  anonKey: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJsbGx4cmpxd21zcmFwdXdrZ3hkeSIsInJlZiI6ImxsbHhyanF3bXNyYXB1d2tneGR5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODYzNzYxMjcsImV4cCI6MjEwMTk1MjEyN30.wejKS6WsJfLCnKuA3_ZKm224SIXjZSydSebU_WAVODY"
};
...
(function(){
  ...
  let scriptLoading=false,observer=null;
  function bind(){
    ...
    if(button.dataset.redImporterBound!=='1'){
      ...
      button.addEventListener('click',function(e){e.preventDefault();e.stopImmediatePropagation();handler();},true);
    }
  ...
  function load(){
    ...
    scriptLoading=true;
    const s=document.createElement('script');s.id='red-word-import-script';s.src='./red-word-import.js?v=7';s.onload=()=>{scriptLoading=false;bind();};s.onerror=()=>{scriptLoading=false;console.error('Không tải được red-word-import.js');};document.head.appendChild(s);
  }
  function ensure(){if(!document.getElementById('word'))return;if(!bind())load();}
  function start(){ensure();if(!observer){observer=new MutationObserver(()=>{if(bind())return;ensure();});observer.observe(document.body,{subtree:true,childList:true});setTimeout(()=>{if(observer){observer.disconnect();observer=null;}},5000);}}
