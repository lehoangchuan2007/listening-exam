...
begin
  select jsonb_build_object(
    'id',e.id,'title',e.title,'description',e.description,'duration_minutes',e.duration_minutes,
    'audio_url',e.audio_url,'audio_name',e.audio_name,'questions',e.questions,
...
as $$
  select case
    ...
    when jsonb_typeof(p_value)='object' then coalesce(
      (select jsonb_agg(value order by case when key ~ '^[0-9]+$' then key::integer else 2147483647 end,key)
...
set search_path=public
as $$
declare key jsonb; submitted_answers jsonb; total integer; correct integer:=0; i integer; score_value numeric(5,2); submission_id uuid; attempts integer; max_a integer; s text;
begin
  ...
  where exam_id=p_exam_id and lower(trim(student_name))=lower(trim(p_student_name)) and lower(trim(student_id))=lower(s);
  ...
  values(p_exam_id,trim(p_student_name),s,submitted_answers,correct,total,score_value) returning id into submission_id;
...
drop policy if exists "teachers can upload own listening audio" on storage.objects;
...
drop policy if exists "teachers can delete own listening audio" on storage.objects;
create policy "teachers can upload own listening audio" on storage.objects
  for insert to authenticated
  with check (bucket_id='listening-audio' and public.is_teacher() and (storage.foldername(name))[1]=auth.uid()::text);
...
create policy "teachers can delete own listening audio" on storage.objects
  for delete to authenticated using(bucket_id='listening-audio' and public.is_teacher() and owner_id=auth.uid()::text);
