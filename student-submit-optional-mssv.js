// English Studio - robust student submission bridge v4.
// Fixes mobile/session timing issues without changing the Supabase security model.
(function(){
  ...
  async function getSessionRetry(client){
    for(let i=0;i<8;i++){
      const r=await client.auth.getSession();
      if(r.data?.session)return r.data.session;
      if(i===3){try{const rr=await client.auth.refreshSession();if(rr.data?.session)return rr.data.session}catch(_){} }
  ...
  if(typeof originalLoadExam==='function'){
  ...
  window.submitExam=async function(){
    ...
    if(!client){alert('❌ Chưa khởi tạo kết nối tài khoản. Vui lòng tải lại trang.');return;}
    const session=await getSessionRetry(client);
    if(!session){
      const gate=document.getElementById('studentAuthOverlay');
      if(gate){alert('🔐 Phiên đăng nhập chưa sẵn sàng. Vui lòng đăng nhập trong cửa sổ tài khoản rồi nộp lại bài.');return;}
      alert('🔐 Vui lòng đăng nhập tài khoản sinh viên trước khi nộp bài.');return;
    }
    const meta=session.user?.user_metadata||{};
    const name=String(meta.full_name||'').trim();
    const sid=String(meta.student_id||'').trim();
    if(!name){alert('❌ Tài khoản sinh viên chưa có Họ tên. Vào ☰ → Tài khoản → Cập nhật tài khoản để bổ sung.');return;}
    const id=examId();if(!id){alert('❌ Không xác định được mã đề. Vui lòng tải lại đề.');return;}
    setBusy(true);
    try{
